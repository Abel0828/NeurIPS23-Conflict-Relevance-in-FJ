...
class RGCN(torch.nn.Module):
    ...
    def __init__(self, dataloader, args, logger):
        ...
        self.conv1 = RGCNConv(self.data.x.shape[-1], 32, 1)
        self.conv2 = RGCNConv(32, 16, 1)
        self.feedforward = nn.Sequential(torch.nn.Linear(32, 16), nn.ReLU(), nn.Linear(16, 2))
    ...
    def forward(self, x, edge_index):
        x = F.relu(self.conv1(x, edge_index, torch.ones(edge_index.shape[-1], dtype=torch.long)))
        x = F.dropout(x, training=self.training)
        x = self.conv2(x, edge_index, torch.ones(edge_index.shape[-1], dtype=torch.long))
        return x
